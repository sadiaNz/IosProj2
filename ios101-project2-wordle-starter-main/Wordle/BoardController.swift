  ...
  func resetBoardWithCurrentSettings() {
    // START YOUR CODE HERE
      numTimesGuessed = 0
          ...
          // Iterate through visible cells to clear letters
      for itemIndex in 0..<numItemsPerRow {
          let indexPath = IndexPath(item: itemIndex, section: currRow)
          if let cell = collectionView.cellForItem(at: indexPath) {
              // Assuming you have a UILabel inside the cell to display the letter
              (cell.contentView.subviews.first as? UILabel)?.text = ""
          }
      }
      collectionView.reloadData()
    // END YOUR CODE HERE
  ...
  // Checkpoint: Correctly implementing this function should change the goal word each time the user inputs an entire row of letters
    ...
    var currentGuess: [String] {
        ...
        for itemIndex in 0..<numItemsPerRow {
            ...
            if let cell = collectionView.cellForItem(at: indexPath) {
                // Assuming you have a UILabel inside the cell to display the letter
                let letter = (cell.contentView.subviews.first as? UILabel)?.text ?? ""
                guess.append(letter)
            }
        }
